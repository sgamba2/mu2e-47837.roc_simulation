...
const char* NoteRepo         = "mu2e-xxxxx";
...
//-----------------------------------------------------------------------------
void plot(int Figure, int Print = 0, const char* Format = "pdf") {
  init();
//-----------------------------------------------------------------------------
// Figure 160 : 
//-----------------------------------------------------------------------------
  if (Figure == 160) {
    ...
    TFile* f1 = TFile::Open("/srv/mu2e/data/projects/tracker/vst/hist/trkvst.annex.trk_fragment_ana.105026.hist");
    TH1F* h1 = (TH1F *)f1->Get("TrkFragmentAna/trk/nh_vs_adc_1");

    p.hd[0]                = hist_data_t(h1,"job1","gamba_roc_data");

    p.hd[0].fNewName       = "RUN105026-data";
    // p.hd[0].fRebin         =  1;
    ...
    p.hd[0].fLabelFontSize = 0.04;
     ...
     TFile* f2 = TFile::Open("/mu2e/app/users/sgamba/r001/sara_daqana/run_105026_1.root");//"/srv/mu2e/data/projects/tracker/vst/hist/trkvst.annex.trk_fragment_ana.105023.hist"
     TH1F* a = (TH1F*)f2->Get("//Hist"); 
    TH1F* h2 = (TH1F*)a->FindObject("nh_vs_adc");

    p.hd[1]                = hist_data_t(h2,"job2","gamba_roc_simulation");
    p.hd[1].fNewName       = "RUN105026-MC";
    // p.hd[1].fRebin         =  1;
    ...
    TFile* f1 = TFile::Open("/srv/mu2e/data/projects/tracker/vst/hist/trkvst.annex.trk_fragment_ana.105026.hist");
    TH1F* h1 = (TH1F *)f1->Get("TrkFragmentAna/trk/frag_0/ch_02");
    ...
    p.hd[0].fLabelFontSize = 0.04;
     ...
     TFile* f2 = TFile::Open("/mu2e/app/users/sgamba/r001/sara_daqana/run_105026_1.root");//"/srv/mu2e/data/projects/tracker/vst/hist/trkvst.annex.trk_fragment_ana.105023.hist"
     auto a = f2->Get("//Hist"); 
     auto  a1 = a->FindObject("ch_02");
     TH1F* h2 = (TH1F*)a1->FindObject("time_02");
