...
//-----------------------------------------------------------------------------
  if (Figure == 4100) {
    ...
    const char* dsid   = "su2020.bmum0s5ab0";
    
    p.hd[0]                = hist_data_t(catalog,"su2020",dsid,"spmc_ana.0000","murat_SpmcAna","spmc_3/cth_vs_mom_1");
    p.hd[0].fNewName       = "ele";
    p.hd[0].fRebin         =  1;
    p.hd[0].fLabel         = "#mu^{-} entering the detector envelope"; // "mom";
...
//-----------------------------------------------------------------------------
// Figure 4101 : beam particle momentum at VD10
//-----------------------------------------------------------------------------
  else if (Figure == 4101) {
    plot_data_t          p(1);
    ...
    p.hd[0].fHist->GetXaxis()->SetRangeUser(90,120);
